# Testimonial portraits

Drop each author's photo here, named to match the `photo` path in
`src/data/testimonials.ts`. Square-ish JP/PNG/WebP, ~400px is plenty.

Expected files:
- `nilson-carraro.jpg` — Nilson Rodrigo Carraro
- `joao-lucas.jpg` — João Lucas
- `antonio-santos.jpg` — Antonio Santos
- `alex-costa.jpg` — Alex Costa
- `fernando-jesus.jpg` — Fernando Jesus
- `caio-nardi.jpg` — Caio Marcelo Nardi
- `fabio-gouveia.jpg` — Fabio Santos Goveia
- `everton-dario.jpg` — Everton Dario
- `alex-alcantara.jpg` — Alex Alcantara
- `bruno-fernandes.jpg` — Bruno Fernandes
- `dimi-vargas.jpg` — Dimi Antoni Vargas
- `caroline-vitorino.jpg` — Caroline Vitorino

If a file is missing the Testimonials page falls back to the person's
initials (no broken image), so it's safe to add photos incrementally.
